...
const setMenuData = async (token, syncCode, db2) => {
  ...
  const customerstmt = db2.prepare(
    "INSERT INTO customers (id,web_id,name,number,due_total,status,created_at,updated_at,sync) VALUES (?,?,?,?,?,?,?,?,?)"
  );
  ...
  try {
    ...
    const addonItemVariationstmt = db2.prepare(
      "INSERT INTO addongroup_item_variation (id,addongroup_id,item_variation_id) VALUES (?,?,?) "
    );
    ...
    const userstmt = db2.prepare(
      "INSERT INTO users (id,name,number,otp,email,pg_cust_id,email_verified_at,activated,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)"
    );
    ...
    const pricesstms = db2.prepare(
      "INSERT INTO prices (id,name,type,status,created_at,updated_at) VALUES(?,?,?,?,?,?)"
    );
    ...
    const attributesstmt = db2.prepare(
      `INSERT INTO attributes (id, name, status, pp_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)`
    );

    const areastmt = db2.prepare(
      "INSERT INTO areas (id,restaurant_id,restaurant_price_id,area,status) VALUES (?,?,?,?,?)"
    );
