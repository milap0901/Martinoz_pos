const updateDatabaseSchema = (latestDbVersion, db2) => {
  try {
    const currentDbVersionRow = db2
      .prepare("SELECT value FROM startup_config WHERE name='db_version'")
      .get();
    ...
    if (currentDbVersion !== latestDbVersion) {
      const updateMaps = [
        ...
        },
        {
          ...
          action: async () => {
            // Function to add a new column
            const addTableTry = `CREATE TABLE try (	id INT PRIMARY KEY,	name VARCHAR(255), age INT,	email VARCHAR(255)) `;
            await db2.prepare(addTableTry).run();
            console.log("Column 'salary' added.");
          },
        },
        {
      ...
      db2.transaction(async () => {
        for (const map of updateMaps) {
          if (
            map.targetVersion > currentDbVersion &&
            map.targetVersion <= latestDbVersion
          ) {
        ...
        db2
          .prepare(
            "UPDATE startup_config SET value = ? WHERE name = 'db_version'"
          )
          .run(latestDbVersion);
      })();
    }
