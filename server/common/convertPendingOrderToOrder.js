...
const convertPendingOrderToOrder = (pendingOrder, updatedStatus, biller) => {
  const itemStmt = db2.prepare(
    "SELECT id,category_id,name,price,item_tax,price_type,parent_tax FROM items WHERE id = ?"
  );
  ...
  const toppingStmt = db2.prepare(
    "SELECT price FROM addongroupitems WHERE id=?"
  );
  ...
  try {
    const orderCart = orderDetail.order_item.map((item) => {
      ...
      const itemTaxArray = itemDetail.item_tax
        ? itemDetail.item_tax.split(",")
        : [];
      ...
      if (item.variation_id === null) {
        if (pendingOrder.restaurant_price_id === null) {
          basePrice =
            itemDetail.price_type === 1
              ? itemDetail.price / (1 + totalTaxPercent / 100)
              : itemDetail.price;
        } else {
          const restaurantPrice = itemRestaurantPriceStmt.get([
            item.item_id,
            pendingOrder.restaurant_price_id,
          ]);
          basePrice =
            itemDetail.price_type === 1
              ? restaurantPrice.price / (1 + totalTaxPercent / 100)
              : restaurantPrice.price;
        }
      } else {
        const variationPrice = variationStmt.get([
          item.item_id,
          item.variation_id,
          pendingOrder.restaurant_price_id,
        ]);
        basePrice =
          itemDetail.price_type === 1
            ? variationPrice.price / (1 + totalTaxPercent / 100)
      ...
      if (item.addongroupitems.length) {
        toppings = item.addongroupitems.map((topping) => {
          ...
          const price =
            itemDetail.price_type === 1
              ? +toppingData.price / (1 + totalTaxPercent / 100)
              : +toppingData.price;
      ...
      const itemTax = item_tax.map((tax) => {
        const itemTaxAmount =
          (itemTotal - item_discount) * (tax.tax / 100) * +item.quantity;
        totalCartTaxAmount += itemTaxAmount;

        let existingTax = cartTaxDetail.find(
          (totaltax) => totaltax.id === +tax.id
        );
        if (existingTax) {
      ...
      return {
        ...
        discount_detail: item.discount_details
    ...
    const finalOrder = {
      ...
      subTotal,
      cartTotal:
        subTotal + totalCartTaxAmount - +orderDetail.order.discount_total,
      orderCart,
