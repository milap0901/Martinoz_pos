...
const createKot = (order, userId, orderId) => {
  ...
  db2.transaction(() => {
    const latestEntry = db2
      .prepare("SELECT created_at, token_no FROM kot ORDER BY ID DESC LIMIT 1")
      .get([]);

    const currentDate = new Date().getDate();
    const lastKOTDate = latestEntry
      ? new Date(latestEntry.created_at).getDate()
      : currentDate;

    tokenNo =
      currentDate !== lastKOTDate || !latestEntry
        ? 1
        : latestEntry.token_no + 1;
