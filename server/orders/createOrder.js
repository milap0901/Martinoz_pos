...
const createOrder = (order) => {
  ...
  const lastOrderData = db2
    .prepare(
      "SELECT bill_no, created_at FROM pos_orders ORDER BY created_at DESC LIMIT 1 "
    )
    .get([]);

  const orderNo = lastOrderData?.created_at
    ? generateBillNo(new Date(lastOrderData.created_at), +lastOrderData.bill_no)
    : 1;
  ...
  if (customerContact) {
    const existingUserInfo = db2
      .prepare("SELECT id FROM customers WHERE number = ?")
      .get([customerContact.trim()]);
  ...
  const orderTrans = db2.transaction((userId) => {
    ...
    if (paymentMethod === "multipay") {
      const mutipayPrepare = db2.prepare(
        "INSERT INTO multipays (pos_order_id,payment_type,amount) VALUES (?,?,?)"
      );
    ...
    const cartTrans = db2.transaction((orderCart, orderId) => {
      ...
      orderCart.forEach((item, i, items) => {
        if (!["removed", "cancelled"].includes(item.itemStatus)) {
          ...
          const totalItemTax = itemTax.reduce(
          ...
          main_order_item_id = contains_free_item
