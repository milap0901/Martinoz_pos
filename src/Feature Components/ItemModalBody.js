...
function ItemModalBody({ id, restaurantPriceVariations, totalTax }) {
  ...
  const selectedVariantId = useSelector(
    (state) => state.currentItem.variation_id
  );
  // const defaultPriceType = useSelector(state => state.bigMenu.defaultSettings.default_price_type);
  const defaultPriceType =
    bigMenu?.defaultSettings?.default_price_type || "without_tax";

  const item = menuItems.find((item) => item.id === id);
  const variants = item.variations;
  const addonGroups = variants.find(
    (variant) => variant.variation_id === selectedVariantId
  ).addonGroups;
