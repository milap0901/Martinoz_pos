...
function EditPrinter() {
  ...
  const {
    ...
    queryFn: getPrinters,
    onSuccess: (data) => {
      const printerDetail = data?.find(
        (printer) => parseInt(printerId) === printer.id
      );
      ...
      setPrinterBillOrderType((prev) => {
        return prev.map((orderType) => {
          ...
          const isChecked =
            printerDetail.bill_print_ordertypes.includes(id.toString()) ||
            false;
          const copyCount =
            printerDetail.bill_print_copy_count.split(",")[id - 1] || 1;
      ...
      setPrinterKotOrderType((prev) => {
        return prev.map((orderType) => {
          const id = orderType.id;
          const isChecked = printerDetail.kot_print_ordertypes.includes(
            id.toString()
          );
          const copyCount =
            printerDetail.kot_print_copy_count.split(",")[id - 1];
          return { ...orderType, isChecked, copyCount };
        });
      });
      ...
      setSelectedCategory((prev) => {
        if (printerDetail.kot_print_categories.toString() === "-1") {
          ...
          return {
            allSelected: false,
            selectedCategoryIds: printerDetail.kot_print_categories
              .split(",")
              .map((id) => +id),
          };
        }
      ...
      setSelectedItems((prev) => {
        if (printerDetail.kot_print_items.toString() === "-1") {
          ...
          return {
            allSelected: false,
            selectedItemIds: printerDetail.kot_print_items
              .split(",")
              .map((id) => +id),
          };
        }
  ...
  const handleSave = async (
    printer,
    printerKotOrderType,
    printerBillOrderType
  ) => {
    const billPrintOrderTypes = printerBillOrderType
      .reduce(
        (acc, orderType) =>
          orderType.isChecked ? [...acc, orderType.id] : acc,
        []
      )
      .join(",");
    const billPrintCopyCount = printerBillOrderType
      .map((ordertype) => ordertype.copyCount || 0)
      .join(",");

    const kotPrintOrderTypes = printerKotOrderType
      .reduce(
        (acc, orderType) =>
          orderType.isChecked ? [...acc, orderType.id] : acc,
        []
      ...
      .join(",");
    const kotPrintCopyCount = printerKotOrderType
      .map((ordertype) => ordertype.copyCount || 0)
      .join(",");

    const printCategories =
      selectedCategory.allSelected ||
      !selectedCategory.selectedCategoryIds.length
        ? "-1"
        : selectedCategory.selectedCategoryIds.join(",");
    const printItems =
      selectedItems.allSelected || !selectedItems.selectedItemIds.length
        ? "-1"
  ...
  return (
    ...
    >
      ...
      {isLoading && (
        <div className={styles.editPrinterOptions}> Loading...</div>
      )}
      {!isLoading && (
      ...
      <div className={styles.editPrinterControl}>
        <button
          ...
          onClick={() =>
            handleSave(printer, printerKotOrderType, printerBillOrderType)
          }
        >
          Save
        </button>
