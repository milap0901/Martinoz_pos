...
import styles from "./AssignBill.module.css";
...
function AssignBill() {
  ...
  const {
    ...
    queryFn: getPrinters,
    onSuccess: (data) => {
      const selectedPrinter = data.find(
      ...
      setPrinterBillOrderType((prev) => {
        return prev.map((orderType) => {
          ...
          const isChecked =
            selectedPrinter.bill_print_ordertypes.includes(id.toString()) ||
            false;
          const copyCount =
            selectedPrinter.bill_print_copy_count.split(",")[id - 1] || 1;
    ...
    >
      <header>
        <div className={styles.headerText}>
          {" "}
          Assign Printer to Bill - {printerName}
        </div>
          ...
          <div className={styles.Tabledata}>
            {isLoading ? (
              ...
              printerBillOrderType.map((orderType) => {
                return (
                  <div className={styles.tableRow} key={orderType.order_type}>
