...
function SettleOrderModal({ show, hide, order, orderMutation, isLoading }) {
    ...
    if (order.payment_type === "multipay") {
      setPaymentDetail((prev) => {
        const updatedMultipay = prev.multipay.map((pay) => {
          const existingPay = order.multipay.find(
    ...
    if (name === "customerPaid") {
      setPaymentDetail((prev) => {
        ...
        const customerReturn =
          returnAmount < 0 ? "Amount is less" : `₹ ${returnAmount}`;
  ...
  const handleSettle = async () => {
    await orderMutation({
      ...
      settleAmount:
        paymentDetail.paymentType === "due"
          ? order.total
          : paymentDetail.settleAmount,
  ...
  return (
    ...
    >
      <Modal.Header closeButton className={styles.settleModalHeader}>
        Save & Settle for - {order.dine_in_table_no} [ ₹{" "}
      ...
      <Modal.Body className={styles.settleModalBody}>
          ...
          <Col className={styles.paymentOptions}>
            <div className={styles.option}>
              <input
                ...
                checked={paymentDetail.paymentType === "cash"}
              />
        ...
        {(paymentDetail.paymentType === "cash" ||
          paymentDetail.paymentType === "card") && (
      ...
      <Modal.Footer className={styles.footer}>
        <button
          ...
          disabled={
            isLoading ||
            paymentDetail.multipay[6].amount === "invalid amount" ||
            paymentDetail.customerReturn === "Amount is less"
          }
        >
          Save & Settle
        </button>
