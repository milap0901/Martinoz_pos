...
function OrderView() {
  ...
  const sortType = useSelector(
    (state) => state.bigMenu.defaultSettings.order_view_sort
  );
  ...
  useSocket("orders", (data) => {
    queryClient.setQueryData("liveOrders", (oldData) => ({
      ...oldData,
      data,
    }));
  });
          ...
          {controls.map((control) => {
            return (
              ...
              >
                ...
                {liveViewOrderType === control.name && (
                  <motion.div
                    layoutId="underline"
      ...
      <div className={styles.OrderViewMain}>
        <AnimatePresence>
          {orders?.data.length !== 0
            ? orders?.data
                ?.filter((order) =>
                  liveViewOrderType === "all"
                    ? true
                    : order.order_type === liveViewOrderType
                )
                .sort((a, b) =>
                  sortType === "asc" ? a.id - b.id : b.id - a.id
                )
                .map((order, idx) => (
                  <OrderCard order={order} key={order.id} idx={idx} />
                ))
            : null}
        </AnimatePresence>
      </div>
