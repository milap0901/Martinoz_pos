...
function KOTCards({ KOT, idx }) {
  ...
  const moveKotToCart = (KOT) => {
    ...
    dispatch(
      modifyUIActive({
        restaurantPriceId: KOT.restaurantPriceId,
        activeOrderBtns,
  ...
  const getColor = (type) =>
    type !== "dine_in" ? { backgroundColor: "rgba(116, 116, 0, 0.87)" } : null;
    ...
    >
      ...
      {showOrderExistModal && (
        <KotEditDeniedModal
          ...
          message={
            "Bill is Already generated for this KOT. This KOT can not be edited"
          }
        />
      )}
    </motion.div>
