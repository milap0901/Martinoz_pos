    ...
    if (!["removed", "cancelled"].includes(item.itemStatus)) {
      ...
      for (const tax of item.itemTax) {
        const taxAfterDiscount =
          (item.itemTotal - itemDiscount) * (tax.tax_percent / 100);
        tax.tax = taxAfterDiscount;
        totalTax += taxAfterDiscount * item.itemQty;

        const existingTaxIndexInTaxDetails = updatedTaxDetails.findIndex(
          (detailTax) => detailTax.id === tax.id
        );
        if (existingTaxIndexInTaxDetails === -1) {
          updatedTaxDetails.push({ ...tax, tax: tax.tax * item.itemQty });
        } else {
          updatedTaxDetails[existingTaxIndexInTaxDetails].tax +=
            taxAfterDiscount * item.itemQty;
        }
      }
    }
  }
...
const finalOrderSlice = createSlice({
  ...
  reducers: {
    addOrderItem: (state, action) => {
      ...
      let existingItem = state.orderCart.find(
        (item) =>
          item.itemIdentifier === orderItem.itemIdentifier &&
      ...
      if (state.cartAction === "default") {
        if (existingItem) {
          ...
          existingItem.multiItemTotal =
            existingItem.itemQty * existingItem.itemTotal;
        } else {
    ...
    addItemNotes: (state, action) => {
      ...
      let existingItem = state.orderCart.find(
        (item) => item.currentOrderItemId === currentOrderItemId
      );
    ...
    removeItem: (state, action) => {
      ...
      if (state.cartAction === "default") {
        state.orderCart = state.orderCart.filter(
          (item) => item.currentOrderItemId !== itemId
        );
      } else if (
        state.cartAction === "modifyOrder" ||
        state.cartAction === "modifyKot"
    ...
    holdToFinalOrder: (state, action) => {
      return action.payload.holdOrder;
    ...
    liveOrderToCart: (state, action) => {
      ...
      if (order.payment_type === "multipay") {
        state.multipay = state.multipay.map((pay) => {
          const existingOrderPay = order.multipay.find(
            (orderPay) => orderPay.payment_type === pay.name
          );
          if (existingOrderPay) {
      ...
      state.orderCart = order.items.map((item) => {
        const itemIdentifier = getIdentifier(
          item.item_id,
          item.variation_id,
          item.itemAddons
        );
    ...
    liveKotToCart: (state, action) => {
      ...
      state.orderCart = KOT.items.map((item) => {
        ...
        let toppingsTotal = item.item_addons.reduce(
          (total, topping) => (total += topping.quantity * topping.price),
          0
        );
        ...
        let itemTax = item.item_tax.map((tax) => {
          ...
          let existingTax = taxDetails.find(
            (totaltax) => totaltax.id === tax.tax_id
          );
          if (existingTax) {
