...
function PendingOrderCard({ order, idx, printers, defaultSettings }) {
  const { mutate, isLoading } = usePendingOrderToOrderMutation(
    printers,
    defaultSettings
  );
  ...
  return (
    ...
    >
      <header
        ...
        style={{
          backgroundColor: orderBgColorMap[order?.order_json.order.order_type],
        }}
      >
        ...
        <div className={styles.orderType}>
          {" "}
          {orderTypeMap[order?.order_json.order.order_type]}
        </div>
        {order?.order_json.order.order_type === "dine_in" &&
        order?.order_json.order.table_no ? (
      ...
      <section className={styles.orderDetail}>
        <div>
          <div>
            {order?.order_json.customer?.customer_name} |{" "}
            {order?.order_json.customer?.phone}
          </div>
          <div>
            {order?.order_json.customer?.address
              ? order?.order_json.customer?.address
              : "------------------"}{" "}
          </div>
        </div>
      ...
      <footer className={styles.orderFooter}>
        <div className={styles.total}>
          ₹ {(+order.order_json.order.total).toFixed(2)}
        </div>
        <button
          className={styles.rejectBtn}
          onClick={() =>
            pendingOrderToOrder(order.id, "rejected", order.online_order_id)
          }
          disabled={isLoading}
        >
        ...
        <button
          ...
          onClick={() =>
            pendingOrderToOrder(order.id, "accepted", order.online_order_id)
          }
          disabled={isLoading}
        >
