...
import {
  useIncludeKotAndCreateOrderMutation,
  useUpdateOrderAndCreateKOTMutation,
...
function MultipayModal({
  ...
  const shouldFetchExistingOrder =
    !!tableNumber && finalOrder.cartAction !== "modifyOrder";
  ...
  const {
    mutate: includeKotAndCreateOrderMutate,
  ...
  const {
    mutate: updateOrderAndCreateKOTMutate,
    isLoading: isUpdateOrderAndCreateKOTLoading,
  } = useUpdateOrderAndCreateKOTMutation(printers, hide, finalOrder);

  const { data: { data } = { data: null }, isLoading: isExistingOrderLoading } =
    useGetExistingOrdersQuery(finalOrder, shouldFetchExistingOrder);
  // console.log(data);
  ...
  if (data?.mergedOrderData && !isExistingOrderLoading) {
    ...
    data?.mergedOrderData.existingOrderMultipayDetail.forEach((existingPay) => {
      const matchPay = multipay.find(
        (pay) => pay.name === existingPay.payment_type && pay.name !== "cash"
      );
      if (matchPay) {
    ...
    const diff =
      data?.mergedOrderData.finalUpdatedCartTotal - existingOrderMultipayTotal;
  ...
  useEffect(() => {
    if (!shouldFetchExistingOrder) {
      ...
      orderMultipayData.forEach((orderPay) => {
        const matchPay = multipay.find(
          (localPay) => localPay.name === orderPay.name
        );
        if (matchPay) {
          matchPay.ref.current.value =
            orderPay.amount === 0 ? "" : orderPay.amount;
  ...
  const handleSave = () => {
    ...
    if (data?.type === "kot" && data?.mergedOrderData) {
      ...
      includeKotAndCreateOrderMutate({
        ...finalOrder,
        multipay: multipayDetail,
        printCount: finalOrder.printCount + 1,
      });
      return;
    }
  ...
  return (
    ...
    >
      <Modal.Header closeButton className={styles.modelHeader}>
        {
          <div
            className={styles.currentOrderTotalContainer}
            ref={currentOrderRef}
          >
            <div className={styles.totalTitle}>Current Order</div>
            <div className={styles.totalAmount}>
              ₹ {data?.mergedOrderData?.updatedLatestOrderCartTotal.toFixed(2)}
            </div>
          </div>
        }
        ...
        {
          <div
            className={styles.prevOrderTotalContainer}
            ref={prevOrderTotalRef}
          >
