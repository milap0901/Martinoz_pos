...
function DiscountModal({ show, hide }) {
  ...
  const addDiscount = () => {
    const discountType = flatDiscountRef.current.checked
      ? flatDiscountRef.current.value
  ...
  return (
    <Modal
      ...
      onHide={hide}
      size="md"
      aria-labelledby="contained-modal-title-vcenttyer"
      centered
      animation={false}
      backdrop="static"
    >
      ...
      <Modal.Body className={styles.modalBody}>
        <DiscountDetail
          flatDiscountRef={flatDiscountRef}
          percentDiscountRef={percentDiscountRef}
