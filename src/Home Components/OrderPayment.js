...
function OrderPayment() {
  ...
  const activeOrderBtns = useSelector(
    (state) => state.UIActive.activeOrderBtns
  );
  ...
  const { mutate: printOrderMutate } = usePrintOrderMutation(
  ...
  const { mutate: kotToPrintOrderMutate } = useKotToPrintOrderMutation(
    printers,
    defaultSettings
  );
  ...
  return (
    <div className={styles.orderPayment}>
      ...
      <div
        className={`${styles.paymentModesCheck} d-flex justify-content-center p-2  text-white`}
      >
        <input
          type="checkbox"
          checked={finalOrder.billPaid}
          name="isPaid"
          onChange={handleChange}
        />
      ...
      <div
        className={`${styles.ProcessOrderBtns} d-flex justify-content-center bg-light pt-3 pb-3 flex-wrap`}
      >
        <>
