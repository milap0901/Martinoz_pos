...
function CustomerDetail({ showDetailType }) {
	...
	const handleChange = async e => {
		...
		if (name === "customerContact" && value.length > 1) {
			const { data } = await axios.get(`http://${IPAddress}:3001/api/v2/customers`, { params: { [name]: value } });

			// set autosuggest state for contact
			setSuggestionsContact(data.data);
		} else {
			setSuggestionsContact([]);
		}
		//  API call for auto suggest for name field
		if (name === "customerName" && value.length > 1) {
			const { data } = await axios.get(`http://${IPAddress}:3001/api/v2/customers`, {
				params: { [name]: value },
			});
			setSuggestionsName(data.data);
		} else {
			...
			{showDetailType === "customerDetail" && (
				<motion.div
					...
					<div className={styles.custemerFieldWrapper}>
						...
						/>
						{//customer suggestion component for customer contact
             suggestionsContact.length !== 0 && (
							<CustomerSuggest suggestions={suggestionsContact} setSuggestions={setSuggestionsContact} />
						)}
					</div>
					...
					<div className={styles.custemerFieldWrapper}>
						...
						<input
							...
							onChange={e => handleChange(e)}
						/>
						{ //customer suggest base  customer name
            suggestionsName.length !== 0 && (
							<CustomerSuggest suggestions={suggestionsName} setSuggestions={setSuggestionsName} />
						)}
					</div>
					...
					<div className={styles.custemerFieldWrapper}>
						...
						<input
							...
							className={`${styles.customerLocality}`}
							onChange={e => handleChange(e)}
						/>
					</div>
				</motion.div>
