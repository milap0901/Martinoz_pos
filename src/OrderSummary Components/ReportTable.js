...
function ReportTable({ orders }) {
  ...
  const getColumnTotal = (table, columnId) => {
    return +table
      .getRowModel()
      .rows.reduce((acc, row) => (acc += +row.getValue(columnId)), 0);
  };
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        footer: ({ table }) =>
          (
            getColumnTotal(table, "Total") -
            getColumnTotal(table, "Tax") -
            getColumnTotal(table, "My Amount") +
            getColumnTotal(table, "Discount")
          ).toFixed(2),
      },
      {
        header: "Total (₹)",
      ...
      },
      {
        ...
        id: "Tip + Total",
        accessorFn: (row) =>
          ((+row?.tip || 0) + (+row?.settle_amount || 0)).toFixed(2),
        cell: (info) => (
          <div style={{ textAlign: "right" }}>{info.getValue()}</div>
        ),
        footer: ({ table }) =>
          (
            getColumnTotal(table, "Tip") + getColumnTotal(table, "Total")
          ).toFixed(2),
      },
    ],
    [orders]
